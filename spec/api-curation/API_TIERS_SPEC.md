...
const createUser = pipe(
  (userData) => isNil(userData) 
    ? Result.Err('User data is required')
    : Result.Ok(userData),
...
```typescript
import { 
  Result, map, flatMap, mapError, match,
  schema, resource, pipeline, rules, rule,
  asyncPipe, maybe, pick, merge
} from 'kairo'
...
const createUser = asyncPipe(
  ...
  match({
    ...
    Err: (error) => mapError(
      error, 
      err => `User creation failed: ${err.message}`
    )
  })
)
```
