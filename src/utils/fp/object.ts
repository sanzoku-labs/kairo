...
export const pick = <T extends object, K extends keyof T>(...keys: K[]) => 
  (obj: T): PickKeys<T, K> => {
...
export const omit = <T extends object, K extends keyof T>(...keys: K[]) => 
  (obj: T): OmitKeys<T, K> => {
    const result = { ...obj }
    for (const key of keys) {
      delete result[key]
...
export const path = (pathArray: Path) => 
