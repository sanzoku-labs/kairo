 ...
 */
export const effect = <T>(fn: UnaryFunction<T, void>) => 
  (value: T): T => { 
    fn(value)
    return value 
  }
 ...
 */
export const conditionalEffect = <T>(
  predicate: Predicate<T>,
  fn: UnaryFunction<T, void>
) => (value: T): T => predicate(value) ? effect(fn)(value) : value
 ...
 */
export const asyncEffect = <T>(fn: UnaryFunction<T, Promise<void>>) => 
  (value: T): T => {
 ...
 */
export const measure = <T>(label: string = 'Operation') => 
 ...
 */
export const countCalls = <T>(
  label: string,
  logInterval: number = 100
) => {
  let count = 0
