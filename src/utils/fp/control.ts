 ...
 */
export const cond =
  <T, R>(conditions: Array<[Predicate<T>, UnaryFunction<T, R>]>) =>
  (value: T): R | undefined => {
...
export const condWith =
  <T, R>(conditions: Array<[Predicate<T>, UnaryFunction<T, R>]>, defaultFn: UnaryFunction<T, R>) =>
  (value: T): R => {
    const match = conditions.find(([predicate]) => predicate(value))
 ...
 */
export const when =
  <T>(predicate: Predicate<T>, fn: UnaryFunction<T, T>) =>
  (value: T): T =>
 ...
 */
export const guard =
  <T>(predicate: Predicate<T>, errorMessage: string | ((value: T) => string)) =>
  (value: T): T => {
    if (!predicate(value)) {
      const message = typeof errorMessage === 'string' ? errorMessage : errorMessage(value)
 ...
 */
export const branch =
  <T, R>(predicate: Predicate<T>, onTrue: UnaryFunction<T, R>, onFalse: UnaryFunction<T, R>) =>
  (value: T): R =>
