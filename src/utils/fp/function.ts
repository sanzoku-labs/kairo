 ...
 */
export const resolve = <T, R>(valueOrFn: Resolvable<T, R>) => 
  (input: T): R => typeof valueOrFn === 'function' 
    ? (valueOrFn as UnaryFunction<T, R>)(input) 
    : valueOrFn

/**
 * Resolve all properties of an object that may contain values or functions
 */
export const resolveAll = <T, R extends Record<string, unknown>>(
  config: { [K in keyof R]: Resolvable<T, R[K]> }
) => (input: T): R => {
  const result = {} as R
 ...
 */
export const resolveWith = <T>() => <R extends Record<string, unknown>>(
  config: { [K in keyof R]: Resolvable<T, R[K]> }
) => resolveAll(config)
 ...
 */
export const apply = <T, R>(fn: UnaryFunction<T, R>) => 
  (input: T): [T, R] => [input, fn(input)]

/**
 * Tap function - apply side effect and return original value
 */
export const tap = <T>(fn: UnaryFunction<T, void>) => 
  (input: T): T => { fn(input); return input }

/**
 * Delay execution of a function
 */
export const delay = <T, R>(ms: number, fn: UnaryFunction<T, R>) => 
 ...
 */
export const retry = <T, R>(
  fn: UnaryFunction<T, R | Promise<R>>,
