...
export const SchemaUtils = {
  ...
  merge: <T, U>(schemaA: Schema<T>, schemaB: Schema<U>): Schema<T & U> => {
    return data.schema({
      ...schemaA.shape,
      ...schemaB.shape,
    })
  ...
  partial: <T>(schema: Schema<T>): Schema<Partial<T>> => {
    const shape = Object.fromEntries(
      Object.entries(schema.shape).map(([key, fieldSchema]) => [key, fieldSchema.optional()])
    )
  ...
  pick: <T, K extends keyof T>(schema: Schema<T>, keys: K[]): Schema<Pick<T, K>> => {
    const shape = Object.fromEntries(keys.map(key => [key, schema.shape[key]]))
    return data.schema(shape)
  },

  // Omit specific fields
  omit: <T, K extends keyof T>(schema: Schema<T>, keys: K[]): Schema<Omit<T, K>> => {
    const shape = Object.fromEntries(
  ...
  extend: <T, U>(schema: Schema<T>, extension: Record<string, any>): Schema<T & U> => {
    return data.schema({
      ...schema.shape,
      ...extension,
    })
...
const UserUpdateSchema = SchemaUtils.partial(SchemaUtils.omit(ExtendedUserSchema, ['id']))
```
...
export const ValidationUtils = {
  ...
  validateAll: <T>(inputs: unknown[], schema: Schema<T>): Result<ValidationError, T[]> => {
  ...
  validateProgressive: <T>(input: unknown, schemas: Schema<T>[]): Result<ValidationError, T> => {
...
export const Transformers = {
  ...
  object: {
    ...
    rename:
      <T>(mapping: Record<string, string>) =>
      (obj: T): T => {
        const result = { ...obj }
...
const transformExamples = {
  ...
  cleanText: (text: string) =>
    data.transform(text, data.string(), {
      customTransforms: {
  ...
  processNumbers: (numbers: number[]) =>
    data.transform(numbers, data.array(data.number()), {
      customTransforms: {
  ...
  deepTransform: <T>(
    ...
    const transform = (obj: any, currentPath: string[] = []): any => {
      ...
      if (Array.isArray(obj)) {
        return obj.map((item, index) => transform(item, [...currentPath, index.toString()]))
      }
    ...
    return Object.fromEntries(
      Object.entries(groups).map(([key, group]) => [
        key,
        {
          ...
          count: group.length,
          ...Object.fromEntries(
            Object.entries(aggregations).map(([aggKey, aggFn]) => [aggKey, aggFn(group)])
          ),
        },
      ])
    )
...
export const QualityUtils = {
  ...
  assessCompleteness: <T>(
    ...
    const overall =
      Object.values(fieldCompleteness).reduce((sum, val) => sum + val, 0) / requiredFields.length
  ...
  cleanData: <T>(
    ...
    try {
      ...
      if (cleaningRules.removeDuplicates) {
        ...
        cleaned = cleaned.filter(item => {
          const key = cleaningRules.removeDuplicates!.map(field => String(item[field])).join('|')
      ...
      cleaned = cleaned.map(item => {
        ...
        Object.entries(cleanedItem).forEach(([key, value]) => {
          ...
          if (cleaningRules.normalizeCase && typeof value === 'string') {
            cleanedItem[field] = (
              cleaningRules.normalizeCase === 'lower' ? value.toLowerCase() : value.toUpperCase()
            ) as T[keyof T]
          }
...
export const ExportUtils = {
  ...
  toJSON: <T>(
    ...
    return pretty ? JSON.stringify(processedItems, null, indent) : JSON.stringify(processedItems)
  },
...
export const PerformanceUtils = {
  ...
  debounce: <TArgs extends any[]>(fn: (...args: TArgs) => void, delay: number) => {
...
const safeTransform = <T, U>(input: T, transformer: (input: T) => U, fallback: U): U => {
  try {
