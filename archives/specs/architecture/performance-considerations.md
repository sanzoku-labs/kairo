...
export const mapResult = <E, T, U>(result: Result<E, T>, fn: (value: T) => U): Result<E, U> => {
...
class ServiceClient {
  ...
  async batch(requests: BatchRequest[]): Promise<BatchResult[]> {
    ...
    const byDomain = requests.reduce(
      (acc, req) => {
...
const buildRequest = (url: string, options: ServiceOptions): OptimizedRequest => {
...
const bundleAnalysis = {
  ...
  checkBundleSize(bundleName: string, actualSize: number) {
    ...
    if (actualSize > maxSize) {
      throw new Error(`Bundle ${bundleName} exceeds size limit: ${actualSize} > ${maxSize} bytes`)
    }
