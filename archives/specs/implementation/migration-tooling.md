...
const transform: Transform = (fileInfo: FileInfo, api: API, options: Options) => {
      ...
      if (args.length >= 1 && j.Literal.check(args[0])) {
        ...
        if (j.ObjectExpression.check(options)) {
          const methodProp = options.properties.find(
            prop =>
              j.Property.check(prop) && j.Identifier.check(prop.key) && prop.key.name === 'method'
          )
  ...
  const hasServiceImport = root.find(j.ImportDeclaration).some(path => {
...
const transform: Transform = (fileInfo: FileInfo, api: API) => {
  ...
  Object.entries(fieldTransforms).forEach(([oldPattern, newPattern]) => {
    ...
    root
      ...
      .forEach(path => {
        const newCall = j.callExpression(
          j.memberExpression(j.identifier(newObj), j.identifier(newMethod.replace('()', ''))),
...
const transform: Transform = (fileInfo: FileInfo, api: API) => {
  ...
  root
    ...
    .filter(path => {
      ...
      return j.MemberExpression.check(callee) && isPipelineChain(callee)
    })
    .forEach(path => {
...
const isPipelineChain = (node: any): boolean => {
  if (j.CallExpression.check(node.object)) {
    return (
      isPipelineChain(node.object.callee) ||
...
export const repository = (name: string, options: any = {}) => {
  console.warn(
    `Repository "${name}" is deprecated. Consider migrating to service + data + pipeline composition.`
  )
...
```typescript
// V1
const result = pipeline().input(data).map(transform).filter(predicate).execute()
